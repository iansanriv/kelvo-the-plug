...
exports.handler=async(event)=>{
  ...
  // Total number of pairs in the cart
  const pairCount = [...requested.values()]
    .reduce((total, qty) => total + qty, 0);

  // $12 for the first pair
  const baseShipping = Math.max(
    0,
    Number(process.env.SHIPPING_BASE_CENTS || 1200)
  );

  // +$5 for every additional pair
  const additionalPairShipping = Math.max(
    0,
    Number(
      process.env.SHIPPING_ADDITIONAL_PAIR_CENTS || 500
    )
  );

  const shippingCents =
    baseShipping +
    Math.max(0, pairCount - 1) * additionalPairShipping;
