...
exports.handler=async(event)=>{
  ...
  try{
    const s=db();
    const {data,error}=await s.from('products').select('id,name,brand,description,condition,badge,image_url,active,created_at,product_variants(id,size,price_cents,stock,reserved_stock,active)').eq('active',true).order('created_at',{ascending:false});
